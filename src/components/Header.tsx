    ...
    return (
        <header className="fixed top-0 left-0 right-0 h-20 bg-slate-900/80 backdrop-blur-md border-b border-slate-800 z-[100] px-6 flex items-center justify-between">
            <div 
                ...
                aria-label="Watch1Do1 Home"
            >
                <img src={APP_LOGO_PATH} alt="" className="h-10 w-auto" aria-hidden="true" />
                <span className="text-xl font-black text-white tracking-tighter">{APP_NAME}</span>
            </div>
            
            <nav className="flex items-center gap-6" aria-label="Main Navigation">
                <button onClick={onDebugClick} className="text-slate-500 hover:text-white transition-colors p-2" title="Debug Panel" aria-label="Search or Debug">
                    <SearchIcon className="w-4 h-4 opacity-50" />
                </button>
                
                <button onClick={onAnalyzeClick} className="hidden md:flex items-center gap-2 text-slate-400 hover:text-white transition-colors" aria-label="AI Vision Scanner">
